...
- **`RLMConfig.main_lm_kwargs` / `sub_lm_kwargs` (`RLM_MAIN_LM_KWARGS` / `RLM_SUB_LM_KWARGS`), a
  ...
  32768 `max_tokens` cap, and inside those calls the median `reasoning_tokens` was the whole budget
  with no content. One such call kills the attempt with `AdapterParseError`, which reads as a model
  that cannot follow the schema, the misdiagnosis 1.10.0 documented under `max_tokens`. Raising the
  cap buys a longer runaway. A `thinking_token_budget` of 16384 gave 0 of 155 calls at the cap, and
  every call cut AT the budget still produced a usable turn.

  **The kit ships the mechanism and no vocabulary, and that is the decision not to re-litigate.** A
  named `RLM_THINKING_BUDGET` would need a provider table that drifts, and it would promise that one
  word means the same thing everywhere. That promise fails inside a single model family: on that
  model `reasoning_effort`, the one name litellm maps across providers, moved reasoning the WRONG
  way (`low` 2443-2562 tokens against 2237 at the default) while breaking the output-format
  instruction in front of a JSON adapter. It stays reachable as a top-level passthrough key with
  litellm's own semantics; the kit does not put its name on it.

  **Two levels.** A top-level key is a litellm parameter and gets litellm's per-provider mapping; a
  key under `extra_body` goes RAW into the request body, which is what a server-specific name needs.
  ...
  **Keys `configure()` owns are refused at parse time** (`model`, `api_key`, `base_url`,
  ...
  not "everything the kit sets" but whether the TRACE can see the override: those five are recorded
  nowhere, so a silent one leaves a trace that reads like a run that went somewhere else.
  `max_tokens` IS readable off the LM, so it is allowed, and
  `RLM_SUB_LM_KWARGS='{"max_tokens":4096}'` is how a per-role generation cap works with no second
  config field.

- **`run_end.payload.budgets.thinking`**: `{"main"/"sub": {"value": int, "key": str}}`, where `key`
  is the dotted path the value was found at. Additive within trace/v1.

  Its own key rather than a field on `budgets.main`, because `budgets.main`'s PRESENCE means "this
  role carried a token cap": a `main` holding only a thinking budget would change what an existing
  reader's `budgets["main"]["cap"]` may assume.

  **The name table is on the READ path only.** The kit sends whatever the caller wrote, so a name it
  does not recognise still reaches the server and is merely not annotated. A stale write-path table
  costs a broken request; a stale read-path one costs an optional trace field. **Absent means NOT
  RECOGNISED, never "no budget".**

  This is also the only answer to the feature's blind spot: **a passthrough cannot report that the
  server ignored a key**, since ignored and honoured are identical on the wire. Pairing
  `budgets.thinking` with `usage`'s `reasoning_tokens` makes that checkable on every run.

- **A warning when a thinking budget is `>=` the role's generation cap**, and another when a
  passthrough lands on a role `configure()` did not build. Both warn and never fail: the first
  combination is inert, and the second is the silence the `request_timeout_s` warning exists for.

### Documented

- **The guide's "Per-role LM request parameters" section carries a measured quoting matrix.** JSON
  in an env var is where this goes wrong, and the layers disagree: a shell-sourced file needs the
  single quotes, `docker run --env-file` never strips them and receives them as part of the
  value, while compose's `env_file` and python-dotenv strip them and accept every form. No single
  spelling survives all four, and single-quoted is the right default.

- **Every em-dash is gone from the repo's prose**, docs, docstrings, comments, workflows and hook
  scripts alike, replaced by ordinary punctuation. No behaviour depends on it; the 30 dashes that
  sat inside runtime strings (log warnings, error messages, one pydantic `Field(description=)` the
  model reads) changed wording only.
